...
```{r}
River_pH <- read.csv("data/River_pH.csv")
```
...
Read in the Palmer Penguins dataset [penguins.csv](./data/penguins.csv).
...
Odds ratios above 1 mean crabs are more likely to be present(because 0 which is absence is the baseline). Odds ratios below 1 mean crabs are less likely to be present. 
...
For numeric predictors like distance, a positive odds ratio such as 1.03 would mean that a 1 unit increase in the predictor, increases the odds of the response being present by 1.03. 
However, an odds ratio for a predictor might be negative which is more difficult to put into words and relate back to the research question.  One solution is to express it as the % decrease. For example, (0.97–1) * 100 = -3%. Then we can write "Each additional increase of one in concentration (a predictor) is associated with an 3% decrease in the odds of a fly surviving."
