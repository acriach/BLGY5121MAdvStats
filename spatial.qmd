...
If you get stuck you can use this source -[seabird dataset](./data/seabirds.csv)


```{r}
#| echo: false
#| eval: false
seabirds <- read.table(file = "data/seabirds.csv", header = TRUE, sep = ",")
seabirds$maxDepth <- seabirds$max_depth.m
seabirds <- subset(seabirds, select = -max_depth.m) # change variable name
library(ncf) 
model1 <- lm(maxDepth ~ species, data=seabirds)
summary(model1)
